...
# Uncomment to compile the clients, libs and broker with XIA socket API support
WITH_XIA:=yes
...
##
## For XIA's sake, it's better to compile mosquitto this with C++ (due to 
...
# Notice that '?=' only sets a varibale if it doesn't contain a value already, 
...
##
## XIA specific variables (not sure fi these should be put here)
##
#XIA_DIR=~/Workbench/xia/xia-core
#XIA_API_DIR=$(XIA_DIR)/api
#XIA_BIN_DIR=$(XIA_DIR)/bin
#XIA_ETC=$(XIA_DIR)/etc
#XIA_LIB=$(XIA_API_DIR)/lib
#XIA_INC=$(XIA_API_DIR)/include

#
# mosquitto binaries will be created in a common 'bin' folder
#
MOSQ_BIN_DIR:=$(XIADIR)/applications/mqtt/mosquitto-1.3.4/bin

LIB_CFLAGS:=${CFLAGS} ${CPPFLAGS} -I. -I.. -I../lib -I$(XINC) -DWITH_XIA
LIB_CXXFLAGS:=$(LIB_CFLAGS) ${CPPFLAGS}
LIB_LDFLAGS:=${LDFLAGS}

BROKER_CFLAGS:=${LIB_CFLAGS} ${CPPFLAGS} -DVERSION="\"${VERSION}\"" -DTIMESTAMP="\"${TIMESTAMP}\"" -DWITH_BROKER
CLIENT_CFLAGS:=${CFLAGS} ${CPPFLAGS} -I../lib -DVERSION="\"${VERSION}\"" -DWITH_XIA
...
ifeq ($(UNAME),Linux)
	...
	#
	# Add paths to XIA compiled libraries for linkage, as specified in 
	# the xia.mk file.
	#
	ifeq ($(WITH_XIA),yes)
		ifeq ($(wildcard $(XLIB)/libXsocket.a),)	
			LIB_LIBS:=$(LIB_LIBS) $(XLIB)/libXsocket.so $(XLIB)/libdagaddr.so $(XLIB)/libXssl.so
			BROKER_LIBS:=$(BROKER_LIBS) $(XLIB)/libXsocket.so $(XLIB)/libdagaddr.so $(XLIB)/libXssl.so
		else
			LIB_LIBS:=$(LIB_LIBS) $(XLIB)/libXsocket.a $(XLIB)/libdagaddr.a $(XLIB)/libXssl.a -Bstatic -lprotobuf
			BROKER_LIBS:=$(BROKER_LIBS) $(XLIB)/libXsocket.a $(XLIB)/libdagaddr.a $(XLIB)/libXssl.a -Bstatic -lprotobuf
		endif
	endif
endif
